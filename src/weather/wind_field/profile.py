...
plt.tight_layout()
plt.savefig(f"Plots/sample_trajectory.png", dpi=300, bbox_inches='tight')
plt.show()



def plot_trajectories_with_highlighted_first(mission_file, output_file='sample_trajectory_select.png'):
    # Load airport database
    airports = load('IATA')
    ...
    ax.set_extent([-125, -66.5, 24.5, 49.5], crs=ccrs.Geodetic())
    ...
    # Plot arcs
    geod = Geod(ellps="WGS84")
    for i, ((lon1, lat1), (lon2, lat2)) in enumerate(od_pairs):
        points = geod.npts(lon1, lat1, lon2, lat2, 100)
        lons = [lon1] + [pt[0] for pt in points] + [lon2]
        lats = [lat1] + [pt[1] for pt in points] + [lat2]

        if i == 0:
            ax.plot(lons, lats, transform=ccrs.Geodetic(), color='red', linewidth=2.5, alpha=1.0, label='First trajectory')
        else:
            ax.plot(lons, lats, transform=ccrs.Geodetic(), color='gray', linewidth=1.5, alpha=0.4)
...
plot_trajectories_with_highlighted_first('sample_missions_AACES.json')
